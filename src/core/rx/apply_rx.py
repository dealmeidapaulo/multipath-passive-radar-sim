...
def apply_rx(static: StaticField, rx) -> StaticField:
    ...
    Parameters
    ----------
    static : StaticField   (reached_cpu/anchors/anchor_ids must be empty)
    rx     : Receiver
    ...
    #
    # For segment (ray_id, seg_id):
    #   A = pos_cpu[seg_id,   ray_id]
    #   B = pos_cpu[seg_id+1, ray_id]
    # Find the point on segment AB closest to rx_pos; test if within rx_rad.
    cands   = np.array(candidates, dtype=np.int32)  # (K, 2)
    rids    = cands[:, 0]
    sids    = cands[:, 1]

    # Keep only valid segments (within the ray's recorded length)
    ...
    for rid in unique_rids:
        n   = int(static.n_pts_cpu[rid])
        pts = [static.pos_cpu[j, rid, :].astype(np.float64) for j in range(n)]
        arr = static.dir_cpu[n-1, rid, :].astype(np.float64)
        r   = Ray(
            transmitter_id = int(static.tx_ids_cpu[rid]),
            points         = pts,
            arrival_dir    = arr,
            frequency      = float(fc),
            power_dbm      = float(static.step_powers[n-1, rid]),
        )
