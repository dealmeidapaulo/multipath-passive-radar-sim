...
def make_street_scene(
    ...
    buildings = make_street_grid(
        ...
        tall_shrink=tall_shrink, seed=seed,
    )
    return Scene(
        box=Box(np.zeros(3), np.array([domain_x, domain_y, domain_z])),
        transmitters=[Transmitter(np.array(tx_pos, dtype=float), frequency, tx_power_w=tx_power_w, tx_id=0)],
        receiver=Receiver(np.array(rx_pos, dtype=float), radius=2.0),
        uav=UAV(np.array(uav_pos, dtype=float), np.array(uav_vel, dtype=float), uav_radius),
        obstacles=buildings,
        n_rays=n_rays,
        n_max=n_max,
        use_physics=True
